...
- 최근 기록 UI의 기본 단위는 개별 이벤트가 아니라 세션이다. 같은 session id의 출근/퇴근을 한 `Session Strip` 카드 안에 묶고, 기본 상태에서는 날짜, 상태, 업무 유형, 총 시간, `출근 -> 퇴근` 시간 범위, 메모 미리보기만 보여준다. `IN/OUT` endpoint 편집과 `세션 삭제`는 사용자가 세션을 펼친 뒤에만 노출한다. 삭제 버튼을 목록 기본 표면에 항상 보이게 되돌리면 안 된다.
