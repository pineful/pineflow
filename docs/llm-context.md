...
- Dribbble `futuristic-ui` 리서치 이후 전체 UI 표면은 `Quiet Telemetry Glass` 방향을 따른다. 딥 그린 상단 계기판, 얇은 luminous border, 낮은 shadow, 미세한 grid, 정돈된 data card는 허용하지만, cyberpunk/Web3식 네온, 항공 HUD 조준선, 자동 scanline/shimmer/pulse/line draw, 과한 blur나 3D 장식은 Pineflow와 맞지 않는다.
