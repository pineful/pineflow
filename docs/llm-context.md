...
- 태블릿 이상 화면에서는 상단 패널을 가로형 대시보드로 구성한다. 왼쪽에는 큰 흐름 그래프, 오른쪽에는 현재 시각, 현재 날씨 요약/상세 펼침, 기록 메모/모드, 출근/퇴근 CTA를 하나의 `dashboardCommandStack`으로 붙여 둔다. 오른쪽 요소를 그래프 높이에 맞춘 여러 grid row로 분산하면 시계와 커맨드 사이에 큰 빈 공간이 생기므로 되돌리지 않는다. 날씨 상세가 펼쳐져도 왼쪽 그래프 카드는 같이 늘어나면 안 되며, 긴 예보는 weather deck 내부 스크롤로 처리한다.
