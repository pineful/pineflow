...
- 최근 기록 UI의 기본 단위는 개별 이벤트가 아니라 세션이다. 같은 session id의 출근/퇴근을 한 `Session Strip` 카드 안에 묶는다. 기본 상태에서는 날짜, 상태, 업무 유형, 총 시간, 진행 레일, 메모 미리보기만 보여주고 정확한 출근/퇴근 시각은 반복 노출하지 않는다. 펼친 영역은 기본 행의 반복이 아니라 정확한 `IN/OUT` 시각, 전체 메모, endpoint 편집, `세션 삭제`처럼 새로 알거나 조작할 수 있는 것만 제공해야 한다. 삭제 버튼을 목록 기본 표면에 항상 보이게 되돌리면 안 된다.
