...
- 최근 기록 UI의 기본 단위는 개별 이벤트가 아니라 세션이다. 같은 session id의 출근/퇴근을 한 `Session Strip` 카드 안에 묶고, `IN -> OUT`, 총 시간, 상태, 업무 유형, 메모를 한 번만 보여준다. 시간 수정은 해당 `IN` 또는 `OUT` endpoint를 누르는 방식으로 진입하고, 삭제는 `세션 삭제`로만 표현한다.
