...
- 기록 수정 화면을 compact command editor로 재배치해 업무 유형, 날짜, 시간, 빠른 보정, 메모, 저장/취소가 넓은 화면에서 불필요하게 벌어지지 않도록 정리.
...
- 보안 점검 결과를 정리해 Cognito가 사용자 비밀번호를 담당하고 DynamoDB에는 기록/설정/캐시만 저장한다는 점을 `docs/security.md`에 명확히 문서화.
- 로그인 email 보조값을 `sessionStorage`로 옮기고, legacy `localStorage` email key를 자동 제거하도록 개선.
- `.env*` ignore와 PoC 환경 예시 placeholder를 정리해 실제 secret이 GitHub에 올라갈 가능성을 낮춤.
...
- Trend Lens 겸임교수 공고에 전용 `academic` scope와 2시간 EventBridge 스케줄을 추가했다(ADR 0012). 운영 배포 후 검증에서 (1) 서울 리전 Lambda가 hibrain CloudFront geo차단을 통과함(`hibrain-recruitment` `ready` 관측)을 확인했고, (2) `scope=all` 전체 수집이 외부 fetch 과다로 8초 timeout 경계에서 다수 source가 `unavailable`로 떨어지고 manual refresh가 간헐 500을 내는 구조적 과부하를 확인했다. 이에 겸임교수 보드(외대+hibrain+겸임 Google News)만 가볍게 갱신하는 `academic` scope를 `security`와 같은 패턴으로 분리해, hibrain이 과부하에 밀리지 않고 안정적으로 수집되게 했다. Lambda timeout 8초 가드레일은 유지하고 수집 작업량을 줄였다. `npm run verify` 통과.
...
- Trend Lens source 상태 details에서 Google Trends 같은 `planned` 소스를 `후보`로 표현하고, 실제 반영된 KISA/CISA 소스 뒤에 배치하도록 정리했다.
