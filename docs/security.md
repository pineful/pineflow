...
- 브라우저 `localStorage`: 비용 사용량 당일 캐시, Trend Lens 읽음 상태, 효과음 설정, Pineflow 탭 안의 최근 활동 ISO 시각처럼 secret이 아닌 UI 보조 상태만 저장한다. 활동 스냅샷은 OS 전체 사용 기록이 아니며, 자동 퇴근 보정 후보 계산에 필요한 최근 시각만 보관한다.
...
- Google News RSS의 `news.google.com/rss/articles/...` 중간 URL은 사용자 클릭 링크로 사용하지 않는다. Lambda는 Google News article/decode endpoint만 allowlist로 제한 조회해 publisher 원문 URL을 우선 얻는다. publisher URL은 화면 이동용으로만 저장하며 Lambda가 해당 URL을 추가 fetch하지 않는다. decode가 실패한 항목에만 Google News 검색 fallback을 사용한다.
