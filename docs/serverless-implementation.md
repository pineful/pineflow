...
첫 화면 `/api/state`는 전체 보관함 조회가 아니라 dashboard bootstrap 조회로 유지한다. DynamoDB `1 RCU` 환경에서 초기 로딩 실패가 반복되지 않도록 `SETTINGS`, `ACTIVE_SESSION`, 최근 `SESSION#` query를 순차 실행하고, 최근 세션 12개와 필요한 projection만 읽는다. 더 많은 과거 기록 조회는 `/api/state` limit를 키우지 않고 별도 archive API로 설계한다.
