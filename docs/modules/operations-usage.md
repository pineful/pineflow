...
- 시각 스타일은 `Quiet Telemetry Glass`의 하단 telemetry panel로 유지한다. 운영 사용량은 기능상 보조 정보지만, 밝은 일반 카드로 남아 있으면 첫 화면의 새 스타일과 분리되어 보이므로 딥 그린 shell과 밝은 상세 데이터 슬롯 대비를 사용한다.
